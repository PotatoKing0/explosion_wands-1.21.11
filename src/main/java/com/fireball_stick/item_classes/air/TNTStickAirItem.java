...
public class TNTStickAirItem extends Item {
    public TNTStickAirItem(Item.Properties properties) {
        super(properties);
    }
    ...
    public InteractionResult use(Level level, Player player, InteractionHand hand) {
        if (!level.isClientSide()) {
            PrimedTnt customTnt = TNTStickClickAir.asPrimedTnt(this, level, player, hand);
            if (customTnt != null) {
                level.addFreshEntity(customTnt);
            }
        }
        return TNTStickClickAir.use(this, level, player, hand);
    }
}
